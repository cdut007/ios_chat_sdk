...
open class ConnectionConfiguration {
    ...
    /**
        Set server tls to which we should connect (ie. if there is no SRV records and server uses port other than default false)
        - parameter directTls: tls to connect to
        */
       open func setDirectTls(_ directTls: Bool?) {
           self.sessionObject.setUserProperty(SocketConnector.DIRECT_TLS, value: directTls);
       }
}
